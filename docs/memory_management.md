...
### 3.2 双池证据缓存 (Dual-Pool Evidence Cache)
- **缓存结构**: 当前 `recent_evidence_cache` 每轮记录同时保存：
  - `final_chunks`: 该轮最终真正进入回答上下文的高精度证据块
  - `candidate_pool`: 该轮在最终截断前仍可供 follow-up 本地扩池的宽候选池
- **兼容策略**: 旧缓存若只有 `chunks` 字段，读取时自动视作 `final_chunks`；`candidate_pool` 为空。
- **follow-up 使用方式**:
  - `reuse_only` 继续主要消费 `final_chunks`
  - `reuse_and_search` / `/deepen` 先读最近 2 轮 `final_chunks` 做 Stage 1 rerank；若命中 broad follow-up、`/deepen`、chunk 太少或 `target_span` 覆盖不足，再扩到最近若干轮 `candidate_pool`
- **落库策略**: 每轮回答结束后，系统会把最终 `pack.chunks` 写入 `final_chunks`，并把 main / gap / agent 运行期收集到的宽候选池重排后写入 `candidate_pool`。

### 3.3 置顶记忆 (Pinned Context)
...
- **返回信息**:
    ...
    - **Cache 权重**: 缓存中每轮 `final_chunks` / `candidate_pool` 的数量、命中权重以及 `is_pinned` 状态。
